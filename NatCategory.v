...
Section NatCat.
  Definition OmegaCategory : @SpecializedCategory nat le.
    refine {| Compose' := (fun _ _ _ H0 H1 => le_trans H1 H0);
      Identity' := le_refl
    |};
    abstract (intros; apply proof_irrelevance).
  Defined.

  Let NatCategory' n : @SpecializedCategory { m | m <= n } (fun x y => proj1_sig x <= proj1_sig y).
