...
Section DataMigrationFunctors.
  ...
  Section Π.
    ...
    Section pre_functorial.
      ...
      Let RightPushforwardAlong_pre_pre_Functor_helper_ObjectOf (g : S ^ C) (d : D) :
        d ↓ F
        -> (IdentityFunctor LocallySmallCat) (fst (projT1 (((CosliceCategoryProjectionFunctor C D) F) d))).
        intro x;
        refine (_ : CommaSpecializedCategory_ObjectT _ _);
        exact (x : CommaSpecializedCategory_ObjectT _ _).
      Defined.

      Let RightPushforwardAlong_pre_pre_Functor_helper_MorphismOf (g : S ^ C) (d : D) s0 d0 (m0 : Morphism (d ↓ F) s0 d0) :
        Morphism ((IdentityFunctor LocallySmallCat) (fst (projT1 (((CosliceCategoryProjectionFunctor C D) F) d))))
                 (RightPushforwardAlong_pre_pre_Functor_helper_ObjectOf g s0)
                 (RightPushforwardAlong_pre_pre_Functor_helper_ObjectOf g d0).
        refine (_ : CommaSpecializedCategory_MorphismT _ _).
        exists (unit_eq _ _, snd (proj1_sig m0)).
        exact (proj2_sig m0).
      Defined.
      ...
      Definition RightPushforwardAlong_pre_pre_Functor (g : S ^ C) (d : D) : SpecializedFunctor (d ↓ F) S.
        ...
        exists (@RightPushforwardAlong_pre_pre_Functor_helper_ObjectOf g d)
               (@RightPushforwardAlong_pre_pre_Functor_helper_MorphismOf g d);
          simpl.
        intros; expand; apply f_equal; simpl_eq; try apply proof_irrelevance.
        expand.
        compute.
        unfold RightPushforwardAlong_pre_pre_Functor_helper_MorphismOf.
        simpl.
        unfold DiscreteCategory_Compose.
        unfold CosliceSpecializedCategory.
        unfold IdentityFunctor, ObjectOf at 6.
        unfold CosliceCategoryProjectionFunctor.
        simpl.
        unfold SliceSpecializedCategory_Functor.
        simpl.
        match goal with
          | [ |- SpecializedFunctor ?C ?D ] => cut (C -> D)
        end.
        Focus 2.
        intro x; hnf.
        refine (_ : CommaSpecializedCategory_ObjectT _ _).
        ...
        exact c.
        Show Proof.
 destruct x as [x]; constructor; exact x.
        Show Proof.
        simpl.
        unfold
        cbv iota.
        ...
        eexists
        simpl.
        eexists (fun x => x) (fun s d m => m).
               (MorphismOf (IdentityFunctor _)).
        eexists (@id _).
        Opaque CosliceCategoryProjectionFunctor.
        simpl in *.
        Transparent CosliceCategoryProjectionFunctor.
        unfold CosliceCategoryProjectionFunctor in *.
        Opaque SliceSpecializedCategoryOver.
        simpl in *.
        unfold ObjectOf in *.
